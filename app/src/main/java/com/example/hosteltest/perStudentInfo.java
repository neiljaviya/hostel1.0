...
public class perStudentInfo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        tempholder.gmark = getIntent().getStringExtra("value4");
        ...
        tv4.setText(tempholder.gmark);
        tv5.setText(tempholder.phone);

        verifybtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                float gm = Float.parseFloat(tempholder.gmark);
                gm = gm*100;
                int ip = (int)gm;
                tempholder.setStatus(true);
                reff1.child(String.valueOf(ip)+" "+(tempholder.id)).child("status").setValue(tempholder.status);
